...
- Always end with encouragement.`;
    ...
    const lines = [
      {
        ...
        spoken:
          lang === "tok" ? "Kauntim wantaim mi!" : "Let's count together!",
      },
      {
        text: lang === "tok" ? "Wan… Tu…" : "One… Two…",
        spoken: lang === "tok" ? "Wan… Tu…" : "One… Two…",
      },
      {
        text: lang === "tok" ? "Tri… Foa… Faiv!" : "Three… Four… Five!",
        spoken: lang === "tok" ? "Tri… Foa… Faiv!" : "Three… Four… Five!",
      },
      {
        text: lang === "tok" ? "Yu save!" : "You got it!",
        spoken: lang === "tok" ? "Yu save!" : "You got it!",
      },
    ];
  ...
  function startSpeechRecognition() {
    ...
    rec.onresult = (e: any) => {
      live = Array.from(e.results as any[])
  ...
  const sendToAI = useCallback(
    async (text: string) => {
      ...
      try {
        ...
        const reply: string =
          data.content?.[0]?.text ?? "Yu save tumas! Keep trying!";
        enterSpeaking(reply);
      } catch {
  ...
  const avatarSize = isActivity ? 60 : 200;

  // ── State → animation map ─────────────────────────────────────────────────
  const avatarAnimate = (() => {
    if (voiceState === "counting")
      return { y: [0, -10, 0], scale: [1, 1.07, 1] };
    if (voiceState === "speaking")
      return { y: [0, -12, 0, -8, 0], scale: [1, 1.06, 1, 1.04, 1] };
  ...
  const avatarTransition = (() => {
    if (voiceState === "counting")
      return { duration: 1.1, repeat: Infinity, ease: "easeInOut" as const };
    if (voiceState === "speaking")
  ...
  const bubbleText =
    voiceState === "listening" && question ? `"${question}"` : response;
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        onClick={() => {
          if (!wasDraggingRef.current) handleTap();
        }}
      >
        ...
        {/* Always-on outer glow rings — make Kumi unmissable */}
        {!isActivity &&
          [0, 1, 2].map((i) => (
            <motion.div
              key={i}
              animate={{
                ...
                opacity: [0.55 - i * 0.15, 0, 0.55 - i * 0.15],
              }}
              transition={{
                duration: 2.2,
                repeat: Infinity,
        ...
        <div
          style={{
            ...
            boxShadow: `0 0 0 6px rgba(255,215,0,0.25), 0 0 40px 12px rgba(255,215,0,0.45), 0 8px 32px rgba(0,0,0,0.7)`,
            background: "radial-gradient(circle at 50% 60%, #1a3a20, #0a1a10)",
